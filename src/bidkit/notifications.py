...
class NotificationVerifier:
    ...
    The client needs only application credentials (the public key is fetched with a
    client-credentials token on the base scope). ``verify`` returns False for a bad or
    malformed signature — respond ``412``; it raises only on operational failures
    (key fetch, unsupported key type) — respond ``500`` and let eBay retry.
    """
    ...
    def verify(self, body: bytes, signature_header: str) -> bool:
        ...
        key_and_hash = self._cache.get(kid)
        if key_and_hash is None:
            response = self._client.commerce.notification.get_public_key(kid)
...
class AsyncNotificationVerifier:
    ...
    async def verify(self, body: bytes, signature_header: str) -> bool:
        ...
        key_and_hash = self._cache.get(kid)
        if key_and_hash is None:
            response = await self._client.commerce.notification.get_public_key(kid)
            key_and_hash = _load_public_key(response.key, response.digest)
            self._cache.set(kid, key_and_hash)
        return _verify(key_and_hash, signature, body)
